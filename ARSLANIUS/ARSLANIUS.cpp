...
[[noreturn]] void load_error(string_view code) {
	if (code == "17") {
		system("cls");
		system("color 0f");
		cout << "======================================================================================================================" << endl;
...
void executeLine(const string& line) {
	...
	if (command == "print") {
		...
		if (timeout >= 300 || timeout <= 0) {
			cout << "ERROR: Setting a wait time of more than 5 minutes is prohibited." << endl;
		}
		else {
			wait(timeout);
		}
		...
		if (eq_pos != string::npos) {
			string file_name = trim(args.substr(0, eq_pos));
			string file_val = trim(args.substr(eq_pos + 1));
			string FileWriteName;
			string varWrite;

			if (!file_name.empty() && file_name.find("Settings And System Files") == string::npos && file_name.find("Users") == string::npos) {
				FileWriteName = file_name;
			}
			else {
				cout << "ERROR: The file/path name cannot be empty or contain \"Settings And System Files\" or \"Users\"" << endl;
				throw ScriptEnd{};
			}
			if (file_name.find(':') != string::npos || file_name.find("..") != string::npos) {
				cout << "ERROR: Absolute paths and '..' are forbidden" << endl;
				throw ScriptEnd{};
			}
			if (!file_val.empty() && file_val.front() == '"' && file_val.back() == '"') {
				varWrite = file_val.substr(1, file_val.length() - 2);
			}
			else {
				lock_guard<mutex> lock(ram_mutex);
				if (fake_ram.find(file_val) != fake_ram.end()) {
					fake_ram[file_val].last_access = chrono::steady_clock::now();
					varWrite = fake_ram[file_val].value;
				}
				else {
			...
			try {
				appendFile(rootPath + "\\" + FileWriteName, varWrite);
			}
			catch (const exception& e) {
		...
		if (eq_pos != string::npos) {
			...
			string file_name = trim(args.substr(eq_pos + 1));
			string filecontent;
			if (file_name.empty() || file_name.find("Users") != string::npos) {
				cout << "ERROR: The file/path name cannot be empty or contain \"Users\"" << endl;
				throw ScriptEnd{};
			}
			...
			lock_guard<mutex> lock(ram_mutex);
			fake_ram[var_name] = { filecontent, chrono::steady_clock::now() };
		}
	}
		...
		if (eq_pos != string::npos) {
			...
			try {
				writeFile(rootPath + "\\" + FileWriteName, varWrite);
			}
			catch (const exception& e) {
				cout << "Filesystem error: " << e.what() << endl;
				throw ScriptEnd{};
...
void setColor(const string& colorCode) {
	...
	int color;
	try {
		color = stoi(colorCode, nullptr, 16);
	}
	catch (...) {
		color = 7;
	}
	...
	COORD home = { 0, 0 };
	DWORD written;
	FillConsoleOutputAttribute(hConsole, (WORD)color, cells, home, &written);
}
...
void bsod(string_view code) {
	...
	if (code == "1a") {
		setColor("17");
		print_slow("*** STOP: 0x00000001a [0xc00000001a, 0x00000000, 0x00000000, 0x00000000]", 2);
		cout << endl;
		print_slow("*** File: \\Settings And System Files", 2);
		cout << endl;
		print_slow("CONFIG_ROOT_NOT_FOUND - The config root is missing.", 2);
		print_slow("Please reinstall or run Startup Repair.", 2);
		cout << endl;
		print_slow("If this is the first time you've seen this error, restart the system.", 2);
		cout << endl;
		print_slow("For support, visit: https://github.com/Armsoup/ARSLANIUS_C-Plus_Plus/issues", 2);
		pause();
		recoveryEnv();
	}
	else if (code == "1") {
		setColor("17");
		print_slow("*** STOP: 0x00000001 [0xc00000001, 0x00000000, 0x00000000, 0x00000000]", 2);
		cout << endl;
		print_slow("*** File: \\Settings And System Files\\SAM", 2);
		cout << endl;
		print_slow("KERNEL_NOT_FOUND - The SAM is missing.", 2);
		print_slow("Please reinstall or run Startup Repair.", 2);
		cout << endl;
		print_slow("If this is the first time you've seen this error, restart the system.", 2);
		cout << endl;
		...
		setColor("17");
		print_slow("*** STOP: 0x00000002 [0xc00000002, 0x00000000, 0x00000000, 0x00000000]", 2);
		cout << endl;
		print_slow("*** File: \\Settings And System Files\\SAM", 2);
		cout << endl;
		print_slow("SYSTEM_ACCOUNT_HASH_MISMATCH - Someone's been playing with SAM in Notepad, huh?", 2);
		print_slow("Please reinstall or run Startup Repair.", 2);
		cout << endl;
		print_slow("Technical information:", 2);
		print_slow("*** Expected system hash: " + EXPECTED_SYSTEM_HASH, 2);
		cout << endl;
		print_slow("If this is the first time you've seen this error, restart the system.", 2);
		cout << endl;
		print_slow("For support, visit: https://github.com/Armsoup/ARSLANIUS_C-Plus_Plus/issues", 2);
		...
		setColor("17");
		print_slow("*** STOP: 0x00000003 [0xc00000003, 0x00000000, 0x00000000, 0x00000000]", 2);
		cout << endl;
		print_slow("*** File: \\Settings And System Files\\REG.cfg", 2);
		cout << endl;
		print_slow("REGISTRY_VERSION_MISMATCH - The version specified in the REG.cfg file is incorrect.", 2);
		print_slow("Please update(7) it to continue working.", 2);
		cout << endl;
		print_slow("Technical information:", 2);
		print_slow("*** Expected version: " + REG_VERSION, 2);
		print_slow("*** Found version: " + REG_VERSION_FOUND, 2);
		cout << endl;
		print_slow("If this is the first time you've seen this error, restart the system.", 2);
		cout << endl;
		print_slow("For support, visit: https://github.com/Armsoup/ARSLANIUS_C-Plus_Plus/issues", 2);
		...
		setColor("17");
		print_slow("*** STOP: 0x00000004 [0xc00000004, 0x00000000, 0x00000000, 0x00000000]", 2);
		cout << endl;
		print_slow("*** File: \\Settings And System Files\\REG.cfg", 2);
		cout << endl;
		print_slow("REGISTRY_NOT_FOUND - The REG.cfg is missing.", 2);
		print_slow("Please reinstall or run Startup Repair.", 2);
		cout << endl;
		print_slow("If this is the first time you've seen this error, restart the system.", 2);
		cout << endl;
		...
		setColor("04");
		print_slow("*** STOP: 0x00000005 [0xc00000005, 0x00000000, 0x00000000, 0x00000000]", 2);
		cout << endl;
		print_slow("*** File: \\Settings And System Files\\SAM", 2);
		cout << endl;
		print_slow("RESERVED_USERNAME_DETECTED - Security violation!", 2);
		print_slow("Someone tried to create 'BarOS AUTHORITY' in SAM.", 2);
		print_slow("That's like printing your own \"100% REAL OFFICIAL\" dollar bill.", 2);
		cout << endl;
		print_slow("Technical information:", 2);
		print_slow("*** Reserved username found in kernel space.", 2);
		print_slow("*** System halted to prevent unauthorized access.", 2);
		cout << endl;
		print_slow("If you call a support specialist, tell him this information so that he can laugh.", 2);
		cout << endl;
		print_slow("For support, visit: https://github.com/Armsoup/ARSLANIUS_C-Plus_Plus/issues", 2);
		pause();
		recoveryEnv();
		...
		setColor("04");
		print_slow("*** STOP: 0x00000006 [0xc00000006, 0x00000000, 0x00000000, 0x00000000]", 2);
		cout << endl;
		print_slow("*** File: \\ARSLANIUS.exe", 2);
		cout << endl;
		print_slow("CRITICAL_STRUCTURE_CORRUPTION - Someone decided to use Debugger, ya?", 2);
		print_slow("It didn't work, cheater.", 2);
		cout << endl;
		print_slow("Technical information:", 2);
		print_slow("*** CRITICAL_STRUCTURE_CORRUPTION", 2);
		cout << endl;
		print_slow("If this is the first time you've seen this error, restart the system.", 2);
		cout << endl;
		print_slow("For support, visit: https://github.com/Armsoup/ARSLANIUS_C-Plus_Plus/issues", 2);
		...
		setColor("17");
		print_slow("*** STOP: 0x00000007 [0xc00000007, 0x00000000, 0x00000000, 0x00000000]", 2);
		cout << endl;
		print_slow("*** File: \\Settings And System Files\\REG.cfg", 2);
		cout << endl;
		print_slow("BAD_SYSTEM_CONFIG_INFO - The registry is missing required entries.", 2);
		print_slow("OS_NAME, SYSTEM_COLOR, ADMIN_COLOR, USER_COLOR, ENABLE_LUA, LOCKDOWN, ADMIN_USER, SETUP or REG_VERSION", 2);
		print_slow("is missing. Run update(7) to restore the registry.", 2);
		cout << endl;
		print_slow("Technical information:", 2);
		print_slow("*** Registry file found but incomplete.", 2);
		print_slow("*** Expected entries: OS_NAME, SYSTEM_COLOR, ADMIN_COLOR, USER_COLOR,", 2);
		print_slow("*** ENABLE_LUA, LOCKDOWN, ADMIN_USER, SETUP REG_VERSION", 2);
		cout << endl;
		print_slow("If this is the first time you've seen this error, restart the system.", 2);
		cout << endl;
		print_slow("For support, visit: https://github.com/Armsoup/ARSLANIUS_C-Plus_Plus/issues", 2);
		...
		setColor("17");
		print_slow("*** STOP: 0x00000008 [0xc00000008, 0x00000000, 0x00000000, 0x00000000]", 2);
		cout << endl;
		print_slow("*** File: \\Settings And System Files\\SAM", 2);
		cout << endl;
		print_slow("KERNEL_INCOMPLETE - The kernel is missing required SYSTEM or ADMINISTRATOR entries.", 2);
		print_slow("Someone deleted important lines from SAM. Probably with Notepad.", 2);
		cout << endl;
		print_slow("Technical information:", 2);
		print_slow("*** Missing: SYSTEM or SYSTEM ADMINISTRATOR account", 2);
		print_slow("*** Kernel file found but incomplete.", 2);
		cout << endl;
		print_slow("If this is the first time you've seen this error, restart the system.", 2);
		cout << endl;
		print_slow("For support, visit: https://github.com/Armsoup/ARSLANIUS_C-Plus_Plus/issues", 2);
		...
		setColor("17");
		print_slow("*** STOP: 0x00000009 [0xc00000009, 0x00000000, 0x00000000, 0x00000000]", 2);
		cout << endl;
		print_slow("*** File: \\ARSLANIUS.exe", 2);
		cout << endl;
		print_slow("LOGON_ATTACK_DETECTED - Too many failed login attempts.", 2);
		print_slow("A brute force attack may be in progress.", 2);
		cout << endl;
		print_slow("Technical information:", 2);
		print_slow("*** Failed attempts: 10", 2);
		print_slow("*** System halted to prevent unauthorized access.", 2);
		print_slow("*** Run Recovery Environment to investigate.", 2);
		cout << endl;
		print_slow("If this is the first time you've seen this error, restart the system.", 2);
		cout << endl;
		print_slow("For support, visit: https://github.com/Armsoup/ARSLANIUS_C-Plus_Plus/issues", 2);
		...
		setColor("17");
		print_slow("*** STOP: 0x00000010 [0xc00000010, 0x00000000, 0x00000000, 0x00000000]", 2);
		cout << endl;
		print_slow("*** File: \\Settings And System Files\\SAM", 2);
		cout << endl;
		print_slow("The kernel is full, run startup repair to reset it.", 2);
		print_slow("Looks like someone needed TOO many accounts.", 2);
		print_slow("Run Startup Repair to restore kernel.", 2);
		cout << endl;
		print_slow("Technical information:", 2);
		cout << "*** Size: " << fileSize / 1024 << " kilobytes" << endl;
		print_slow("*** Kernel file found but locked.", 2);
		cout << endl;
		print_slow("If this is the first time you've seen this error, restart the system.", 2);
		cout << endl;
		print_slow("For support, visit: https://github.com/Armsoup/ARSLANIUS_C-Plus_Plus/issues", 2);
		...
		setColor("17");
		print_slow("*** STOP: 0x00000011 [0xc00000011, 0x00000000, 0x00000000, 0x00000000]", 2);
		cout << endl;
		print_slow("*** File: \\Settings And System Files\\REG.cfg", 2);
		cout << endl;
		print_slow("The registry is full, run startup repair to reset it.", 2);
		print_slow("Looks like someone filled the registry with all sorts of junk, huh?", 2);
		print_slow("Run Startup Repair to restore registry.", 2);
		cout << endl;
		print_slow("Technical information:", 2);
		cout << "*** Size: " << fileSize / 1024 << " kilobytes" << endl;
		print_slow("*** REG file found but locked.", 2);
		cout << endl;
		print_slow("If this is the first time you've seen this error, restart the system.", 2);
		cout << endl;
		print_slow("For support, visit: https://github.com/Armsoup/ARSLANIUS_C-Plus_Plus/issues", 2);
		...
		setColor("17");
		print_slow("*** STOP: 0x00000012 [0xc00000012, 0x00000000, 0x00000000, 0x00000000]", 2);
		cout << endl;
		print_slow("*** File: \\Settings And System Files\\system.log", 2);
		cout << endl;
		print_slow("LOG_OVERFLOW - The system log is full of shit.", 2);
		print_slow("Someone's been writing a novel in system.log.", 2);
		print_slow("150 KB is the limit. Run Startup Repair to clear the log.", 2);
		cout << endl;
		print_slow("Technical information:", 2);
		cout << "*** Size: " << fileSize / 1024 << " kilobytes" << endl;
		print_slow("*** Log file found but locked due to size limit.", 2);
		cout << endl;
		print_slow("If this is the first time you've seen this error, restart the system.", 2);
		cout << endl;
		print_slow("For support, visit: https://github.com/Armsoup/ARSLANIUS_C-Plus_Plus/issues", 2);
		...
		setColor("17");
		print_slow("*** STOP: 0x00000013 [0xc00000013, 0x00000000, 0x00000000, 0x00000000]", 2);
		cout << endl;
		print_slow("*** File: \\Settings And System Files\\BCD", 2);
		cout << endl;
		print_slow("BCD_CORRUPTED - The BCD is missing required entries.", 2);
		print_slow("DEFAULT_MODE, BOOT_TIMEOUT, FAST_BOOT or DRIVER_LOAD_OFF is missing.", 2);
		print_slow("Run update(7) to restore the BCD.", 2);
		cout << endl;
		print_slow("Technical information:", 2);
		print_slow("*** BCD file found but incomplete.", 2);
		print_slow("*** Expected entries: BOOT_TIMEOUT, DEFAULT_MODE, FAST_BOOT, DRIVER_LOAD_OFF", 2);
		cout << endl;
		print_slow("If this is the first time you've seen this error, restart the system.", 2);
		cout << endl;
		print_slow("For support, visit: https://github.com/Armsoup/ARSLANIUS_C-Plus_Plus/issues", 2);
		...
		setColor("17");
		print_slow("*** STOP: 0x00000014 [0xc00000014, 0x00000000, 0x00000000, 0x00000000]", 2);
		cout << endl;
		print_slow("*** File: \\Settings And System Files\\BCD", 2);
		cout << endl;
		print_slow("BCD_NOT_FOUND - The BCD is missing.", 2);
		print_slow("Please reinstall or run Startup Repair.", 2);
		cout << endl;
		print_slow("If this is the first time you've seen this error, restart the system.", 2);
		cout << endl;
		...
		setColor("17");
		print_slow("*** STOP: 0x00000015 [0xc00000015, 0x00000000, 0x00000000, 0x00000000]", 2);
		cout << endl;
		print_slow("*** File: \\Settings And System Files\\Drivers", 2);
		cout << endl;
		print_slow("DRIVER_CRITICAL_FAILURE - A critical driver returned BAROS_CRITICAL (2).", 2);
		print_slow("Set DRIVER_LOAD_OFF=1 in BCD to disable all drivers.", 2);
		cout << endl;
		print_slow("If this is the first time you've seen this error, restart the system.", 2);
		cout << endl;
		print_slow("For support, visit: https://github.com/Armsoup/ARSLANIUS_C-Plus_Plus/issues", 2);
		...
		setColor("04");
		print_slow("*** STOP: 0x00000016 [0xc00000016, 0x00000000, 0x00000000, 0x00000000]", 2);
		cout << endl;
		print_slow("*** File: \\ARSLANIUS.exe", 2);
		cout << endl;
		print_slow("CRITICAL_KERNEL_ERROR - A serious problem has occurred and ARSLANIUS has stopped working.", 2);
		print_slow("Try updating the program to the latest version or, conversely, reverting to the old one.", 2);
		cout << endl;
		print_slow("If this is the first time you've seen this error, restart the system.", 2);
		cout << endl;
		print_slow("For support, visit: https://github.com/Armsoup/ARSLANIUS_C-Plus_Plus/issues", 2);
		...
		setColor("17");
		print_slow("*** STOP: 0x00000018 [0xc00000018, 0x00000000, 0x00000000, 0x00000000]", 2);
		cout << endl;
		print_slow("*** File: \\Settings And System Files\\SAM", 2);
		cout << endl;
		print_slow("RESUME_USER_NOT_FOUND - Hibernation user not found; please contact your SYSTEM ADMINISTRATOR", 2);
		print_slow("for assistance with creating and/or restoring the account.", 2);
		cout << endl;
		print_slow("Technical information:", 2);
		print_slow("*** User: " + currentUser, 2);
		cout << endl;
		print_slow("If this is the first time you've seen this error, restart the system.", 2);
		cout << endl;
		print_slow("For support, visit: https://github.com/Armsoup/ARSLANIUS_C-Plus_Plus/issues", 2);
		...
		setColor("17");
		print_slow("*** STOP: CRITICAL_PROCESS_DIED [0xc00000000, 0x00000000, 0x00000000, 0x00000000]", 2);
		cout << endl;
		print_slow("*** File: \\ARSLANIUS.exe", 2);
		cout << endl;
		print_slow("Technical information:", 2);
		...
		setColor("17");
		print_slow("*** STOP: 0xDEADBEEF [0x00000666, 0x00000000, 0x00000000, 0x00000000]", 2);
		cout << endl;
		print_slow("MANUAL_CRASH - You typed 'bsod' and now you're here. Surprised? You shouldn't be.", 2);
		print_slow("This error was intentionally triggered by the 'bsod' command.", 2);
		print_slow("No real damage was done. Just reboot and continue.", 2);
		cout << endl;
		print_slow("Technical information:", 2);
		print_slow("*** Crash initiated by user: " + currentUser, 2);
		print_slow("*** Stop code: 0xTeam_by_" + currentUser, 2);
		print_slow("*** Next time try 'help' instead. Or don't. I'm not your mom.", 2);
		cout << endl;
		print_slow("For support, visit: https://github.com/Armsoup/ARSLANIUS_C-Plus_Plus/issues", 2);
		pause();
		bootMenu();
		...
		setColor("08");
		print_slow("*** STOP: 0x00001225a [0x00000220, 0x00000002, 0x00000000a, 0x00000000]", 2);
		cout << endl;
		print_slow("UNKNOWN_ERROR - Something went wrong and ARSLANIUS crashed,", 2);
		print_slow("perhaps the bsod environment variable was not defined due to a serious problem,", 2);
		print_slow("replace the main ARSLANIUS file with the original one.", 2);
		cout << endl;
		print_slow("Technical information:", 2);
		print_slow("*** UNKNOWN_ERROR", 2);
		print_slow("*** Stop code: 0x00001225a", 2);
		print_slow("*** Last User: " + currentUser, 2);
		print_slow("*** Uptime: " + getUptime(), 2);
		print_slow("*** Build: " + currentBuild, 2);
		print_slow("*** Version Kernel: " + VersionBarOSkrnl, 2);
		cout << endl;
		print_slow("For support, visit: https://github.com/Armsoup/ARSLANIUS_C-Plus_Plus/issues", 2);
		pause();
		exit(1);
...
void safeModeBoot() {
	...
	Sleep(1000);
	if (dirExists(sysProf)) cout << "Loaded: \\Settings And System Files\\systemprofile" << endl;
	Sleep(1000);
...
void startupRepair() {
	...
	// Create SAM
	stringstream kernel;
	kernel << "SYSTEM = " << EXPECTED_SYSTEM_HASH << endl;
	kernel << "SYSTEM ADMINISTRATOR = " << EXPECTED_ADMIN_HASH << endl;
...
void interfaceScreen() {
	...
	clearScreen();
	if (diagnostic == 2 || diagnostic == 1 || rec || currentUser == "BarOS\\KERNEL") setColor("0e");
	if (safeMode) setColor("07");
	...
	// Check autorun
	if (!safeMode && !rec && diagnostic == 0 && currentUser != "BarOS\\KERNEL") {
			...
			if (fileExists(sysServices + "\\TrustedInstaller.active")) {
				...
				if (ins_err == 1) {
					...
					if (!fileExists(kernelPath)) {
						...
						kernel_ins << "SYSTEM = " << EXPECTED_SYSTEM_HASH << endl;
						kernel_ins << "SYSTEM ADMINISTRATOR = " << EXPECTED_ADMIN_HASH << endl;
...
void BarOSkrnl(string_view Kernel_mode) {
	...
	if (Kernel_mode == "loading") {
		...
		auto checkFileSize = [](string_view path, size_t maxSize, string_view bsodCode) -> bool {
			...
			if (hFile != INVALID_HANDLE_VALUE) {
				LARGE_INTEGER fileSizelocal;
				if (GetFileSizeEx(hFile, &fileSizelocal)) {
					if (fileSizelocal.QuadPart > static_cast<LONGLONG>(maxSize)) {
						DWORD64 size = static_cast<DWORD64>(fileSizelocal.QuadPart);
						CloseHandle(hFile);
						fileSize = size;
						bsod(bsodCode.data());
						return false;
		...
		if (!DriverloadOFF) {
			...
			g_api.print_slow = [](const char* text, int delay) { print_slow(text, delay); };
	...
	if (Kernel_mode == "ACPI") {
		if (acpiRequest == 1) {
			if (fastBoot) BarOSkrnl("hibernate");
			exit(0);
		}
		if (acpiRequest == 2) { g_AntiHackRunning = 0; Sleep(650); g_AntiHackRunning = 1; BarOSkrnl("initPath"); bootMenu(); }
		setColor("0f");
		clearScreen();
